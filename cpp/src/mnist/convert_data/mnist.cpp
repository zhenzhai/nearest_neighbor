...
void mnist_generate() {
    ...
    strcpy(filepath, TRAIN_LABEL_PATH);
    fin = fopen(filepath, "rb");
    strcpy(filepath, TRN_VTR_PATH);
    fout1 = fopen(filepath, "wb");
    strcpy(filepath, TRN_LBL_PATH);
    ...
    /* TEST DATA */
    fprintf(stderr, "> converting mnist test data\n");
    fprintf(stderr, "  > populating data buffer for test data\n");
    strcpy(filepath, TEST_VECTOR_PATH);
    fin = fopen(filepath, "rb");
    if (fin == NULL)
    {
        fprintf(stderr, "ERROR: Can't locate the input file.\n");
    ...
    fclose(fin);
    fprintf(stderr, "  > dimensions w: %d, h: %d\n", dataw, datah);
    strcpy(filepath, TEST_LABEL_PATH);
    fin = fopen(filepath, "rb");
    strcpy(filepath, TST_VTR_PATH);
    fout1 = fopen(filepath, "wb");
    strcpy(filepath, TST_LBL_PATH);
    fout2 = fopen(filepath, "wb");
